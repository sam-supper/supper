...
export const Header: FC<HeaderProps> = ({ links, contact, information }) => {
  ...
  return (
    <header 
      ref={headerRef}
      className="w-full fixed top-0 left-0 px-site-x z-[100] py-16 md:pt-16 md:pb-12 bg-translucent backdrop-blur-nav text-nav flex items-center justify-between md:site-grid place-items-start"
    >
      ...
      <Link href="/" className="md:col-span-3">
        <Logo className="w-full h-auto max-w-80 md:max-w-100" />
      </Link>
      ...
      <button className="md:hidden grid-contain w-20 h-20 place-items-center">
        <span className="sr-only">Toggle Menu</span>
        <div className="w-full h-[1.5px] bg-black"></div>
        <div className="w-full h-[1.5px] bg-black rotate-90"></div>
      </button>
    </header>
  )
}
